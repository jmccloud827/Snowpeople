...
@Model final class SnowpersonModel {
    ...
    func getCanMovePartBackwardByID(_ id: ObjectIdentifier) -> Bool {
        guard let index = persistedParts.firstIndex(where: { $0.id == id }) else {
            return false
        }
    ...
    func movePartBackwardByID(_ id: ObjectIdentifier) {
        let index = persistedParts.firstIndex { $0.id == id }
        if let index, index > 0 {
            let partToMove = persistedParts[index]
            let partToMoveBeforeIt = persistedParts[index - 1]
